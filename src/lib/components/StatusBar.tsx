  ...
  return (
    <footer class="flex items-center justify-between px-4 py-1 bg-base-200 border-t border-base-300 text-xs">
      <div class="flex items-center gap-3">
        ...
        {conn.isConnected && (
          <>
            <span class="text-base-content/60">{conn.portName}</span>
      ...
      <div class="flex items-center gap-3 font-mono">
        <span>TX: <span class="text-primary">{formatBytes(txBytes.value)}</span></span>
        <span class="text-base-content/40">|</span>
        <span>RX: <span class="text-secondary">{formatBytes(rxBytes.value)}</span></span>
      </div>
    </footer>
  )
}
