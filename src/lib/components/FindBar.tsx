...
export function FindBar({ open, onClose }: FindBarProps) {
  ...
  return (
    <div class="flex items-center gap-2 px-3 py-1.5 bg-base-200 border-b border-base-300">
      ...
      <div class="join">
        <button class={`btn btn-xs join-item ${mode === 'text' ? 'btn-active' : ''}`} onClick={() => setMode('text')}>Text</button>
        <button class={`btn btn-xs join-item ${mode === 'hex' ? 'btn-active' : ''}`} onClick={() => setMode('hex')}>HEX</button>
      </div>
      <input ref={inputRef} type="text" class="input input-xs input-bordered flex-1 font-mono max-w-xs" placeholder={mode === 'hex' ? '48 65 6C' : 'Search...'} value={query} onInput={(e) => setQuery((e.target as HTMLInputElement).value)} onKeyDown={handleKeydown} />
      {query && <span class="text-xs text-base-content/50">{matchCount} matches</span>}
