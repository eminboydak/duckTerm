...
export function ConnectionBar() {
    ...
    try {
      await invoke('open_port', {
        ...
        config: {
          ...
          data_bits: conn.dataBits,
          parity: conn.parity,
          stop_bits: conn.stopBits,
          flow_control: conn.flowControl
        }
      })
      connection.connect()
    } catch (e) {
  ...
  return (
    <div class="flex items-center gap-2 p-2 bg-base-200 border-b border-base-300">
      ...
      <button
        class={`btn btn-sm ${conn.isConnected ? 'btn-error' : 'btn-success'}`}
        disabled={loading}
